...
from .kuavo_mimic_config import KuavoMimicCfg
...
class KuavoMimic(HumanoidMimic):
    def __init__(self, cfg: KuavoMimicCfg, sim_params, physics_engine, sim_device, headless):
        self.cfg = cfg
    ...
    def _get_body_indices(self):
        # Kuavo 身体部位索引获取
        # 直接根据 URDF 中的名称查找，比模糊匹配更准确
        
        # 上臂 (Upper Arm)
        upper_arm_names = ["zarm_l3_link", "zarm_r3_link"]
        ...
        # 前臂 (Lower Arm)
        lower_arm_names = ["zarm_l5_link", "zarm_r5_link"]
        ...
        # 躯干 (Torso)
        torso_name = [self.cfg.asset.torso_name] # base_link
    ...
    def compute_observations(self):
        ...
        # 强制将踝关节速度观测置为0 (Sim-to-Real 技巧)
        # Kuavo 踝关节索引: Left [4, 5], Right [10, 11] (假设关节顺序为 L_Leg, R_Leg)
